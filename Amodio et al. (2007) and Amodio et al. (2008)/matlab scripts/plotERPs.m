...
for m = 1:length(files)
    ...
    if strcmp(type, "ERN") && epoch_counts.trials(4) >= 6
        %take only those with incorrect no-go responses of 6 or more
        filename_idx(m) = 1;
    
    elseif strcmp(type, "N2") && epoch_counts.trials(2) >= 20
...
ERP = pop_savemyerp(ERP, 'erpname', 'all', 'filename', 'all.erp', 'filepath', [data_path 'forPlotting' type], 'Warning', 'on');
