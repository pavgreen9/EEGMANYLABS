    ...
    for m = 1:height(behav_data)
        ...
        elseif trig_file.type(k+1) == 1 && behav_data.triggers(m) == 1
            ...
            if behav_data.key_resp_corr(m) == 1 && trig_file.type(k+2) == 3
                comp.type(k+1) = 3;             % change this to 0 for response lock
                trig_file.type(k+1) = 3; % change this to 0 for response lock
                
                comp.type(k+2) = 0; % change this to 3 for response lock
                trig_file.type(k+2) = 0; % change this to 3 for response lock
                
            % if resp is untimely, mark as late 5
            elseif behav_data.key_resp_after500_corr(m) == 1 && trig_file.type(k+2) == 3
                comp.type(k+1) = 5;
                trig_file.type(k+1) = 5;
                
                comp.type(k+2) = 0; %5
                trig_file.type(k+2) = 0; %5
